%dessin de la sigmoide pour l'interconnection tout ou rien
%attention aux arguents dans NeuronePourInterconnection: Il est dit qu'on
%utilise un regular spiking


function [B,FRfinal]=SigmoideInterconnectionToutOuRien(B,R,M,tstep,Te,Ts,Tf)
l=length(B);
FRfinal=zeros(1,l);
for k=1:l
   [V,A]=NeuronePourInterconnectionToutOuRien(0.02,0.2,-65,8,B(k),tstep,Te,Ts,M,R);
